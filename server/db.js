...
const tables = [
  ...
  `CREATE TABLE IF NOT EXISTS points (
    ...
    created_at INTEGER NOT NULL DEFAULT (strftime('%s','now')))`,
  // ─── Calendar (Cadence) — every row scoped to the owning user_email ─────────
  `CREATE TABLE IF NOT EXISTS cal_projects (
    id TEXT NOT NULL,
    user_email TEXT NOT NULL COLLATE NOCASE,
    name TEXT NOT NULL,
    color TEXT NOT NULL DEFAULT '#5b4bdb',
  ...
  `CREATE TABLE IF NOT EXISTS cal_items (
    ...
    notes TEXT DEFAULT '',
    recurrence TEXT NOT NULL DEFAULT '{"type":"none","days":[]}',
    backlog INTEGER NOT NULL DEFAULT 0,
    priority TEXT NOT NULL DEFAULT 'none',
    project_id TEXT,
    habit_icon TEXT DEFAULT 'flame',
    created_at INTEGER NOT NULL DEFAULT (strftime('%s','now')),
  ...
  `CREATE TABLE IF NOT EXISTS cal_completions (
    ...
    PRIMARY KEY (user_email, item_id, date))`
];
...
const indexes = [
  ...
  `CREATE INDEX IF NOT EXISTS idx_cal_items_user   ON cal_items(user_email)`,
  `CREATE INDEX IF NOT EXISTS idx_cal_items_date   ON cal_items(user_email, date)`,
  `CREATE INDEX IF NOT EXISTS idx_cal_proj_user    ON cal_projects(user_email)`,
  `CREATE INDEX IF NOT EXISTS idx_cal_comp_user    ON cal_completions(user_email)`
];
